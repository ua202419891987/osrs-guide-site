...
# -*- coding: utf-8 -*-
"""
v3 - 第3版：使用简单字符串替换清理已有的cn-title/cn-summary，再添加正确版本。
"""
import re, os

BASE_DIR = r"C:\Users\Lenovo\osrs-guide-site\zh\guides"
OUT_DIR = r"C:\Users\Lenovo\AppData\Local\Temp\osrs_m_format_v3"
...
def process_file(filepath, cfg):
    ...
    # === 第1步：清理已有cn-title和cn-summary ===
    # 使用正则移除所有cn-title和cn-summary标签
    content = re.sub(r'<h1 class="cn-title[^>]*>.*?</h1>\s*', '', content, flags=re.DOTALL)
    content = re.sub(r'<p class="cn-summary[^>]*>.*?</p>\s*', '', content, flags=re.DOTALL)
    
    # === 第2步：添加正确的cn-title和cn-summary ===
    if not cfg.get("already_has_cn_title", False):
        # 在guide-hero中找英文<h1>
        h1_pattern = re.compile(r'(<section class="guide-hero">[\s\S]*?)(<h1[^>]*>)([\s\S]*?)(</h1>)')
        m = h1_pattern.search(content)
        if m:
            prefix = m.group(1)     # 从<section到<h1之前
            h1_open = m.group(2)
            h1_text = m.group(3)
            h1_close = m.group(4)
            rest = content[m.end():]
            
            cn_h1 = f'<h1 class="cn-title" style="font-size:1.5rem;color:#1a1a1a;margin-bottom:4px;font-weight:700;">{cfg["cn_title"]}</h1>'
            cn_p = f'<p class="cn-summary" style="color:#333;font-size:0.95rem;margin-bottom:16px;line-height:1.6;">{cfg["cn_summary"]}</p>'
            
            # 插入cn-title和cn-summary，保留原有的格式缩进
            content = prefix + cn_h1 + '\n            ' + cn_p + '\n            ' + h1_open + h1_text + h1_close + rest
            print(f"  [OK] 添加了中文H1和导语")
        else:
            print(f"  [WARN] 找不到<h1>")
    
    # === 第3步：添加H2/H3翻译 ===
    def add_translation(heading_tag, mapping):
        if not mapping:
            return content
        def repl(m):
            full = m.group(0)
            ...
            return full
        return re.sub(rf'<{heading_tag}[^>]*>(.*?)</{heading_tag}>', repl, content, flags=re.DOTALL)
    
    content = add_translation('h2', cfg.get("h2_map", {}))
    content = add_translation('h3', cfg.get("h3_map", {}))
    
    # 写入输出
    out = os.path.join(OUT_DIR, os.path.basename(filepath))
