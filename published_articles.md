# OSRS Guru — Published Articles Record

> **用途**：每次写新文章前，AI 会自动读取此文件并对比，防止重复内容发布到 Medium。

---

## Medium 已发布文章

| # | 标题 | 发布时间 | 关键主题 | Medium 链接 |
|---|------|----------|----------|------------|
| 1 | Top 5 OSRS Mistakes Beginners Make (And How to Avoid Them in 2026) | 2026-06-01 (约) | 新手常犯错误 | *(待补充)* |
| 2 | Top 5 OSRS Mistakes Beginners Make (And How to Avoid Them in 2026) | 2026-06-02 (约) | 新手常犯错误（重复发布，建议删除一篇） | *(待补充)* |
| 3 | OSRS Beginner's Roadmap 2026: Your Complete Guide to Old School RuneScape | 2026-06-02 (约) | 新手路线图（仅 11 字，建议扩充或替换） | *(待补充)* |
| 4 | **OSRS F2P to Member Transition Guide 2026: When to Upgrade and How to Afford It** | 2026-06-05 (待发布) | F2P→会员转型决策 + 经济帐 | *(待补充)* |
| 5 | **OSRS Mid-2026 Review: What Shipped, What Delayed, What's Next** | 2026-06-24 (ShipPage已发) | 年中复盘、Sailing/Vault of Ralos 交付评估、Blood Moon 延迟、League 6 预测 | https://shippage.ai/p/lk0slg |
| 6 | **Crimson Desert Beginner Guide 2026: 7 Things I Wish I Knew** | 2026-06-24 (ShipPage已发) | CD 新手入门、战斗、职业、资源管理 | https://shippage.ai/p/e8gqdu |

---

## 已发布文章 — 内容摘要（用于查重）

### #1 & #2 "Top 5 Mistakes..."（两篇相同）
- 主题：新手最常犯的 5 个错误
- 关键内容：训练顺序错误、忽略任务、购买 GP 等
- **注意**：#1 和 #2 标题完全相同，建议删除其中一篇（Medium 可能会标记低质量）

### #3 "Beginner's Roadmap 2026"
- 主题：新手路线图
- 关键内容：仅 11 个单词（疑似草稿误发）
- **建议**：扩充此文或删除，重新发布完整版

### #4 新文 "F2P to Member Transition"
- 主题：F2P 天花板、会员付费方案对比、Bond 经济帐、转型时机
- **与已有文章无重叠**：不涉及"错误"主题，不涉及完整路线图

---

## 查重规则（AI 自动执行）

每次写新 Medium 文章前：
1. 读取此文件
2. 对比新文章标题、关键主题、内容摘要
3. 如果重叠度 >40%，拒绝发布并提示修改
4. 发布后，立即将此文加入此记录表

---

## 待办

- [ ] 补充 #1、#2、#3 的 Medium 文章链接
- [ ] 决定是否删除 #2（与 #1 重复）
- [ ] 决定是否扩充 #3（目前仅 11 字）
- [ ] 发布 #4 后，将链接填入上表
